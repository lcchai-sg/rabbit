...
function generateBrandID(input) {
...
function getWaterResistance(input) {
    if (!input) return "";
...
function getColor(input) {
    ...
    if (input.match(/Cocoa|brown|chocolate|maroon|burgundy/i)) {
        return 'Brown';
    }
    ...
    if (input.match(/red|cherry|rose/i) && !(input.match(/Textured/i))) {
        return 'Red';
    }
    if (input.match(/Siver|Siulver|Silvr|Silver|Silve|Sillver|Silive|Slver|Sliver|Slilver|silver|platin[i]?um|rhodium|titan[i]?um/i)) {
        return 'Silver';
    }
    if (input.match(/white[ -_]?gold/i)) {
        return 'White Gold';
    }
    if (input.match(/Whitw|Whire|Whie|Whitw|Whute|Whtie|Whte|White/i)) {
        return 'White';
    }
...
function getMaterial(input) {
    ...
    if (input.match(/oyster[ -_]?steel|SS|stainless stee|Stainless[ -_]*Steel/i) || (input.match(/mesh/i) && (!(input.match(/alloy[ -_]*mesh/i))))) {
        if (!bm) {
            bm = 'Stainless Steel';
            bt = 'Bracelet';
        }
    ...
    if (input.match(/18[ -_]?(k|kt|ct|karat|carat)[ -_]?(Eve)[ -_]?(rose)|(Eve)[ -_]?(rose)[ -_]?(gold)/i) && (!(input.match(/gold[s]?[ -_]?(((ion)[ -_]?)?plate|ip|grain|tone|pvd|trim|coat)/i)))) {
        if (!bm) {
            bm = 'Everose Gold';
            bt = 'Bracelet';
        }
        ...
        goldAssigned = true;
    } else if (input.match(/18[ -_]?(k|kt|ct|karat|carat)[ -_]?(rose)|(rose)[ -_]*[\(\s*pink\s*\)]?(gold)/i) && (!(input.match(/gold[s]?[ -_]?(((ion)[ -_]?)?plate|ip|grain|tone|pvd|trim|coat)/i)))) {
        if (!bm) {
            bm = 'Rose Gold';
            bt = 'Bracelet';
        }
    ...
    if (!goldAssigned && input.match(/Gold/i) && (!(input.match(/gold[s]?[ -_]?(((ion)[ -_]?)?plate|ip|grain|tone|pvd|trim|coat)/i))) && (!(input.match(/resin|rubber|leather/i)))) {
        if (!bm) {
            bm = 'Gold';
            bt = 'Bracelet';
        }
    ...
    if (input.match(/Alligator\/Crocodile|Alligarot|Alliagtor|Alligaor|Alligaror|(al(l)?igator)[\s-_]*(leather)?|lligator/i) && (!(input.match(/Alligator Motif/i)))) {
        if (bm) {
            if (bm === 'Leather') {
    ...
    if (input.match(/(croc)(o)?(dile)?[\s-_]*(leather)?/i) && (!(input.match(/Alligator\/Crocodile|Crocoi-Emboss|(cro(c)?c)(o)?(dile)?[\s-_]*(patte|emb[r]?oss|texture)/i)))) {
        if (bm) {
            if (bm === 'Leather') {
    ...
    return { bm, bms, bt, unknow: true, };
}

function getCaseShape(input) {
    if (!input) return "";
...
function getGender(input) {
    ...
    if (input.match(/gent|men|man|male/i)) {
        return 'M';
    }
...
function getCrystalCoating(input) {
    if (!input) return "";
...
function getCaseCrown(input) {
    if (!input) return "";

    if (input.match(/non/i) && input.match(/screw/i)) {
        return 'Non-Screw';
    }
...
function getLuminescence(input) {
    if (!input) return "";
...
// function getAttr implements DataMapper<string, any> {
...
module.exports = {
    generateBrandID, getBandType, getBezel, getBuckle,
    getCalendar, getCaliberType, getCaseBack, getCaseCrown, getCaseShape,
    getCoating, getColor, getCrystal, getCrystalCoating, getDialFinish,
    getDialType, getGender, getHandStyle, getIndexType, getLuminescence,
    getMaterial, getStyle, getWaterResistance,
}
