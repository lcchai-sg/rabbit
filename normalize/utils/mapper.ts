...
export class getMaterial implements DataMapper<string, any> {
    map(input: string): any {
        ...
        if (input.match(/SS|stainless stee|Stainless[ -_]*Steel/i) || (input.match(/mesh/i) && (!(input.match(/alloy[ -_]*mesh/i))))) {
            if (!bm) {
                bm = 'Stainless Steel';
                bt = 'Bracelet';
            }
        ...
        if (input.match(/Diamond|Diamon|diamod/i) && !(input.match(/diamond[ -_]?like/i))) {
            bm = bm ? bm : 'Diamond';
            bms.push('Diamond');
        }
