...
const indexing = async (context) => {
    ...
    try {
        ...
        const d = data.match(/<loc>.*<\/loc>/ig);
        const dd = d ? d[0].replace(/<loc>|<\/loc>/ig, '') : null;
        console.log(dd);
        const { data: xml } = await client.get(dd);
        console.log(xml);
        ...
        parser.parseString(xml, (err, res) => {
            for (let i = 1; i < res.urlset.url.length; i++) {
                ...
                console.log(d);
                const url = d['loc'] && d['loc'][0] ? d['loc'][0] : null;
                let name, thumbnail;
                if (d['image:image'] && d['image:image'][0]) {
                    name = d['image:image'][0]['image:title'] && d['image:image'][0]['image:title'][0] ?
                        d['image:image'][0]['image:title'][0] : null;
                    thumbnail = d['image:image'][0]['image:loc'] && d['image:image'][0]['image:loc'][0] ?
                        d['image:image'][0]['image:loc'][0] : null;
                }
                console.log({ url, name, thumbnail });
                if (url.match(/\b\d\d[a-z]\d\d\d\b|\b\d\d[a-z]\d\d\b/i)) {
                    const reference = name ? name.split(' ')[0] : null;
                    result.items['all'].push({
                        source, lang, brand, brandID, url, name, reference, retail: null, thumbnail,
                    })
                }
            }
        });
        ...
        console.error('Failed indexing for Bulova with error : ' + error)
        return {};
    }
};
...
(async () => {
    const r = await indexing({
        client: axios,
        entry: "https://intl.bulova.com/sitemap.xml",
    })
    r && r.items['all'].forEach(w => {
        console.log(w);
    })
