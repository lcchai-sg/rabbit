...
const u = require('./u_zenith_no_assets');
    ...
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url !== u[i]) {
                const out = `${u[i]} ||| 404\n`;
                console.log(`     404 : ${u[i]}`);
                fs.appendFile('zenith_no_assets.err', out, (err) => { if (err) throw err; });
            } else {
                let th = $('.product-header-item-image').find('img').attr('src').split('?')[0];
                if (th) {
                    th = th.split('?')[0];
                    const uu = th.split('/');
                    ...
                    console.log('     ', out);
                    fs.appendFile('zenith_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
