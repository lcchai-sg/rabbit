...
const u = require('./u_tagheuer_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/tagheuer/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (th) {
                ...
                console.log('     ', out);
                fs.appendFile('tagheuer_no_assets.out', out + '\n', (err) => { if (err) throw err; });
            } else {
                const out = `${u[i]} ||| no thumbnail\n`;
                console.log(`     NO THUMBNAIL : ${u[i]}`);
                fs.appendFile('tagheuer_no_assets.err', out, (err) => { if (err) throw err; });
            }
        } catch (err) {
