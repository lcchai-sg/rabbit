...
const u = require('./u_meister_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/meister/';
        ...
        try {
            ...
            if (th && !th.match(/placeholder|coming|unavailable/i)) {
                if (th) {
                    ...
                    console.log('     ', out);
                    fs.appendFile('meister_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
                    const out = `${u[i]} ||| no thumbnail\n`;
                    console.log(`     NO THUMBNAIL : ${u[i]}`);
                    fs.appendFile('meister_no_assets.err', out, (err) => { if (err) throw err; });
                }
            } else {
                const out = `${u[i]} ||| no thumbnail\n`;
                console.log(`     NO THUMBNAIL : ${u[i]}`);
