...
const u = require('./u_frederique_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/frederique/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url.toLowerCase() !== ur.toLowerCase()) {
                ...
                fs.appendFile('frederique_no_assets.err', out, (err) => { if (err) throw err; });
            } else {
                let th = $('.product-page--root').find('meta[property="og:image"]').first().attr('content');
                // if (!th.match(/https/i)) th = "https://www.frederique.com" + th;
                if (th) {
                    th = th.split('?')[0];
                    const uu = th.split('/');
                    ...
                    console.log('     ', out);
                    fs.appendFile('frederique_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
