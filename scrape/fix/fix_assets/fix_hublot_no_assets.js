...
const u = require('./u_hublot_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/hublot/';
    for (let i = 0; i < u.length; i++) {
        console.log(u.length, i, u[i]);
        try {
            await new Promise(r => setTimeout(r, 5000));
            const $ = cheerio.load((await axios.get(u[i])).data);
            let th = $('.js_ma_watch_button').attr('data-image-url');
            if (!th) th = $('meta[property="og:image"]').attr('content');
            const url = $('meta[property="og:url"]').attr('content');
            if (url !== u[i]) {
                const out = `${u[i]} ||| 404\n`;
                console.log(`     404 : ${u[i]}`);
                fs.appendFile('hublot_no_assets.err', out, (err) => { if (err) throw err; });
            }
            if (th) {
                // th = th.split('?')[0];
                const uu = th.split('/');
                ...
                console.log('     ', out);
                fs.appendFile('hublot_no_assets.out', out + '\n', (err) => { if (err) throw err; });
            } else {
