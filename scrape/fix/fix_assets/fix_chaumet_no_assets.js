...
const u = require('./u_chaumet_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/chaumet/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url.toLowerCase() !== u[i].toLowerCase()) {
                const out = `${u[i]} ||| 404\n`;
                console.log(`     404 : ${u[i]}`);
                fs.appendFile('chaumet_no_assets.err', out, (err) => { if (err) throw err; });
            } else {
                let th = $('meta[property="og:image"]').attr('content');
                if (!th.match(/https/i)) th = "https://www.chaumet.com" + th;
                if (th) {
                    th = th.split('?')[0];
                    const uu = th.split('/');
                    ...
                    console.log('     ', out);
                    fs.appendFile('chaumet_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
