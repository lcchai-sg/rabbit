...
const u = require('./u_gseiko_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/gseiko/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url.toLowerCase() !== u[i].toLowerCase()) {
                const out = `${u[i]} ||| 404\n`;
                console.log(`     404 : ${u[i]}`);
                fs.appendFile('gseiko_no_assets.err', out, (err) => { if (err) throw err; });
            }
            let th = $('.okra-carousel-slide-inner').find('img').attr('src');
            ...
            if (th) {
                ...
                console.log('     ', out);
                fs.appendFile('gseiko_no_assets.out', out + '\n', (err) => { if (err) throw err; });
            } else {
