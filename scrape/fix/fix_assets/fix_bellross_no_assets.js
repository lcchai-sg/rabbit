...
const u = require('./u_bellross_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/bellross/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url.toLowerCase() !== u[i].toLowerCase()) {
                ...
                fs.appendFile('bellross_no_assets.err', out, (err) => { if (err) throw err; });
            } else {
                let th = $('#main_img_thum').attr('src');
                if (!th.match(/https/i)) th = "https://www.bellross.com" + th;
                if (th) {
                    th = th.split('?')[0];
                    const uu = th.split('/');
                    ...
                    console.log('     ', out);
                    fs.appendFile('bellross_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
