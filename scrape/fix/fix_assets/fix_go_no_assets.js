...
const u = require('./u_go_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/go/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url.toLowerCase() !== u[i].toLowerCase()) {
                const out = `${u[i]} ||| 404\n`;
                console.log(`     404 : ${u[i]}`);
                fs.appendFile('go_no_assets.err', out, (err) => { if (err) throw err; });
            } else {
                ...
                if (th) {
                    ...
                    console.log('     ', out);
                    fs.appendFile('go_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
