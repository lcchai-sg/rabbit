...
const u = require('./u_alpina_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/alpina/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url.toLowerCase() !== ur.toLowerCase()) {
                ...
                fs.appendFile('alpina_no_assets.err', out, (err) => { if (err) throw err; });
            } else {
                let th = $('meta[property="og:image"]').attr('content');
                // if (!th.match(/https/i)) th = "https://www.alpina.com" + th;
                if (th) {
                    th = th.split('?')[0];
                    const uu = th.split('/');
                    ...
                    console.log('     ', out);
                    fs.appendFile('alpina_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
