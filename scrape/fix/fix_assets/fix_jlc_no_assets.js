...
const u = require('./u_jlc_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/jlc/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (!th) th = "https://www.jaeger-lecoultre.com" + $('.new-pdp-top-views__main__banner').find('img').attr('src');
            const url = $('meta[property="og:url"]').attr('content');
            if (url !== u[i]) {
                const out = `${u[i]} ||| 404\n`;
                console.log(`     404 : ${u[i]}`);
                fs.appendFile('jlc_no_assets.err', out, (err) => { if (err) throw err; });
            }
            if (th) {
                th = th.split('?')[0];
                const uu = th.split('/');
                ...
                console.log('     ', out);
                fs.appendFile('jlc_no_assets.out', out + '\n', (err) => { if (err) throw err; });
            } else {
