...
const u = require('./u_baume_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/baume/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url.toLowerCase() !== u[i].toLowerCase()) {
                ...
                fs.appendFile('baume_no_assets.err', out, (err) => { if (err) throw err; });
            } else {
                let th = "https://www.baume-et-mercier.com" + $('.product-gallery__picture').first().find('img').attr('src');
                if (!th.match(/https/i)) th = "https://www.baume-et-mercier.com" + th;
                if (th) {
                    th = th.split('?')[0];
                    const uu = th.split('/');
                    ...
                    console.log('     ', out);
                    fs.appendFile('baume_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
