...
(async () => {
  ...
  // const u = await db.collection(mdb.coll).distinct("url", { assets: null, code: { $nin: [404, 'not product'] } });
  const fp = "https://synopsis.cosmos.ieplsg.com/files/";
  let cnt = 0; let stop = '';
  for (let i = 0; i < u.length && stop !== 'y'; i++) {
    if (i % 100 === 0) console.log(u.length, i);
    if (u[i]) {
      const p = await db.collection(mdb.coll).distinct("assets", { url: u[i] });
      if (p.length > 1) {
        p.sort();
        let updateAll = true;
        for (let j = 1; j < p.length && updateAll; j++) {
          if (p[j] && p[j] !== p[0]) {
            ...
            await exec(cmd);
            cmd = 'open -a "Google Chrome" "' + fp + p[0] + '"';
            await exec(cmd);
            for (let k = 1; k < p.length; k++) {
              cmd = 'open -a "Google Chrome" "' + fp + p[k] + '"';
              await exec(cmd);
            }
