...
(async () => {
    ...
    for (let i = 0; i < u.length; i++) {
        ...
        if (p && p.length > 0 && p[0]) {
            ...
            fs.appendFile('fix_wmaxx.out', out, (err) => { if (err) throw err; });
        } else {
            ...
            try {
                ...
                const th = $('.MagicZoom ').attr('href');
                // if (url !== u[i]) {
                //     const out = `${u[i]} ||| 404\n`;
                //     fs.appendFile('fix_wmaxx_404.out', out, (err) => { if (err) throw err; });
                // } else {
                if (th && !th.match(/coming|placeholder/i)) {
                    const out = `${u[i]} ||| ${th}\n`;
