...
const u = require('./u_nomos_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/nomos/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url.toLowerCase() !== ur.toLowerCase()) {
                ...
                fs.appendFile('nomos_no_assets.err', out, (err) => { if (err) throw err; });
            } else {
                let th = $('.product-main-img').find('img').attr('data-src');
                // if (!th.match(/https/i)) th = "https://www.nomos.com" + th;
                if (th) {
                    th = th.split('?')[0];
                    const uu = th.split('/');
                    ...
                    console.log('     ', out);
                    fs.appendFile('nomos_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
