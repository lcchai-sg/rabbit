...
const u = require('./u_iwc_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/official/iwc/';
    for (let i = 0; i < u.length; i++) {
        ...
        try {
            ...
            if (url !== u[i]) {
                const out = `${u[i]} ||| 404\n`;
                console.log(`     404 : ${u[i]}`);
                fs.appendFile('iwc_no_assets.err', out, (err) => { if (err) throw err; });
            }
            if (th) {
                th = th.split('?')[0];
                const uu = th.split('/');
                ...
                console.log('     ', out);
                fs.appendFile('iwc_no_assets.out', out + '\n', (err) => { if (err) throw err; });
            } else {
