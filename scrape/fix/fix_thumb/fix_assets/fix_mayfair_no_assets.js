...
const u = require('./u_mayfair_no_assets');


(async () => {
    const fp = '/Users/lcchai/Work/assets/mayfair/';
    for (let i = 0; i < u.length; i++) {
        console.log(u.length, i, u[i]);
        try {
            await new Promise(r => setTimeout(r, 5000));
            const $ = cheerio.load((await axios.get(u[i])).data);
            let th = $(".mt-thumb-switcher").first().attr("href");
            if (th && !th.match(/placeholder|coming|unavailable/i)) {
                if (th) {
                    ...
                    console.log('     ', out);
                    fs.appendFile('mayfair_no_assets.out', out + '\n', (err) => { if (err) throw err; });
                } else {
                    const out = `${u[i]} ||| no thumbnail\n`;
                    console.log(`     NO THUMBNAIL : ${u[i]}`);
                    fs.appendFile('mayfair_no_assets.err', out, (err) => { if (err) throw err; });
                }
            } else {
                const out = `${u[i]} ||| no thumbnail\n`;
                console.log(`     NO THUMBNAIL : ${u[i]}`);
