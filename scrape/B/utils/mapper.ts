...
export class getBuckle implements DataMapper<string, any> {
    map(input: string): any {
        ...
        if (input.match(/tang|buckle|pin/i)) {
            return 'Buckle';
        }
        // cannot identify
        return "";
...
export class getColor implements DataMapper<string, any> {
    map(input: string): any {
        ...
        if (input.match(/diamond[s].*pav|pav.*diamond|diamond-set/i)) {
        ...
        if (input.match(/coffee|\bsable\b|Meteorite|Graphite|anthracite|black|carbon|charcoal/i)) {
            if (input.match(/(Meteorite|Graphite|anthracite|black|carbon|charcoal) *(mother[ -_]*of[ -_]*pearl)/i))
        ...
        if (input.match(/cobalt|aqua|cyan|bue|azure|sapphire|blue|teal|turquoise/i)) {
            if (input.match(/(blue|teal|turquoise) *(mother[ -_]*of[ -_]*pearl)/i))
                return 'Blue Mother of Pearl';
            return 'Blue';
        }
        if (input.match(/Burgandy|chestnut|havana|Cognac|\btan\b|copper|bronze|cocoa|brown|chocolate|maroon|burgundy/i)) {
            if (input.match(/(Cognac|\btan\b|copper|bronze|cocoa|brown|chocolate|maroon|burgundy) *(mother[ -_]*of[ -_]*pearl)/i))
        ...
        if (input.match(/lime|olive|mint|digital|camouflage|green/i)) {
            if (input.match(/green *(mother[ -_]*of[ -_]*pearl)/i))
        ...
        if (input.match(/rainbow|enamel|multi[ -_]?colo[u]?r|multi[ -_]?(ton|strip)/i)) {
            return 'Multi-color';
        }
        if (input.match(/navy/i)) {
            return 'Navy';
        }
        if (input.match(/coral|salmon|orange/i)) {
            return 'Orange';
        }
        ...
        if (input.match(/blush|pink/i)) {
            if (input.match(/(blush|pink) *(mother[ -_]*of[ -_]*pearl)/i))
                return 'Pink Mother of Pearl';
            return 'Pink';
        }
        if (input.match(/plum|lil[l]?ac|Aubergine|lavender|purple|violet/i)) {
            if (input.match(/(Aubergine|lavender|purple|violet) *(mother[ -_]*of[ -_]*pearl)/i))
                return 'Purple Mother of Pearl';
            return 'Purple';
        }
        if (input.match(/\bmop\b|Fuchsia|\bred\b|cherry|rose/i)) {
            if (input.match(/(\bred\b|cherry|rose) *(mother[ -_]*of[ -_]*pearl)/i))
                return 'Red Mother of Pearl';
            return 'Red';
        }
        if (input.match(/mirror|Ruthenium|Siver|Siulver|Silvr|Silver|Silve|Sillver|Silive|Slver|Sliver|Slilver|silver|platin[i]?um|rhodium|titan[i]?um/i)) {
            return 'Silver';
        }
        if (input.match(/(18[ -_]?(k|kt|ct|karat|carat))[ -_]?white|white[ -_]?(gold)/i)) {
            return 'White Gold';
        }
        if (input.match(/Porcelain|ivoy|creem|creme|egg[ -]?shell|\bpearl\b|Cream|ivory|Opaline?|Whitw|Whire|Whie|Whitw|Whute|Whtie|Whte|White/i)) {
            if (input.match(/(Cream|ivory|Opaline|Whitw|Whire|Whie|Whitw|Whute|Whtie|Whte|White) *(mother[ -_]*of[ -_]*pearl)/i))
        ...
        if (input.match(/amber|sun|chrome|yellow/i)) {
            if (input.match(/(yellow) *(mother[ -_]*of[ -_]*pearl)/i))
        ...
        if (input.match(/mother[ -_]*of[ -_]*(pearl|Pear|peal|peral)/i)) {
...
export class getMaterial implements DataMapper<string, any> {
    map(input: string): any {
        ...
        if (input.match(/Sportech|Nato|Textlie|denim|cloth|Frabric|Fabric|Linen|Textile|Farbic|Fabrik/i)) {
            if (!bm) {
                bm = 'Fabric';
                bt = 'Strap';
            }
...
export class getGender implements DataMapper<string, any> {
    map(input: string): any {
        ...
        if (input.match(/gent|\bmen\b|\bman\b|\bmale\b/i)) {
            return 'M';
        }
